...
describe('isQuietHours — overnight window (22:00–08:00 IST)', () => {
...
describe('isQuietHours — edge cases', () => {
  it('handles a same-day window that does not wrap midnight', () => {
    const cfg = { ...DEFAULT_QUIET_HOURS, startMinute: 13 * 60, endMinute: 14 * 60 }
  ...
  it('treats start === end as an empty window, never a 24h one', () => {
    const cfg = { ...DEFAULT_QUIET_HOURS, startMinute: 60, endMinute: 60 }
  ...
  it('fails open when the timezone cannot be resolved', () => {
    ...
    const cfg = { ...DEFAULT_QUIET_HOURS, timeZone: 'Mars/Olympus_Mons' }
  ...
  it('respects a non-IST zone', () => {
    const cfg = { ...DEFAULT_QUIET_HOURS, timeZone: 'UTC' }
...
describe('quietHoursFromEnv', () => {
  it('defaults to 22:00–08:00 Asia/Kolkata', () => {
  ...
  it('turns suppression off entirely', () => {
  ...
  it('falls back to the default for malformed values instead of throwing', () => {
    vi.spyOn(console, 'warn').mockImplementation(() => {})
